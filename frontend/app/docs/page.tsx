...
function Code({ children, lang = 'ts' }: { children: string; lang?: string }) {
  return (
    <div className="border-2 border-[#e9e4f0] overflow-hidden my-5 bg-[#281f38]">
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#3a2f4e]">
        <span className="text-[10px] f-type text-[#9a91ad] uppercase tracking-[0.16em]">{lang}</span>
      </div>
      <pre className="px-5 py-4 text-xs f-type text-[#e9e4f0] overflow-x-auto leading-6 whitespace-pre">{children}</pre>
    </div>
  );
}

function Callout({ type = 'info', children }: { type?: 'info' | 'warn' | 'tip'; children: React.ReactNode }) {
  const border = { info: 'border-[#b794f4]', warn: 'border-[#d9a441]', tip: 'border-[#d9c964]' };
  return (
    <div className={`border-l-2 ${border[type]} pl-4 my-5 py-1`}>
      <div className="f-type text-xs text-[#b3abc4] leading-6">{children}</div>
    </div>
  );
}

function H2({ children }: { children: React.ReactNode }) {
  return <h2 className="f-news font-bold text-[26px] text-[#e9e4f0] tracking-tight mt-12 mb-4 first:mt-0">{children}</h2>;
}
...
function P({ children }: { children: React.ReactNode }) {
  return <p className="f-news text-[15px] text-[#c9c2d6] leading-7 mb-4">{children}</p>;
}

function Table({ rows }: { rows: { f: string; t?: string; d: string }[] }) {
  return (
    <div className="border border-[#3a2f4e] overflow-hidden mb-6">
      <table className="w-full">
        <tbody className="divide-y divide-[#3a2f4e]">
          {rows.map((r) => (
            <tr key={r.f} className="hover:bg-[#2d2440] transition-colors">
              <td className="px-4 py-3 f-type text-[11px] text-[#e9e4f0] align-top w-36 shrink-0">{r.f}</td>
              {r.t && <td className="px-4 py-3 f-type text-[11px] text-[#b794f4] align-top w-20">{r.t}</td>}
              <td className="px-4 py-3 f-type text-[11px] text-[#9a91ad] leading-5">{r.d}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function Rows({ items }: { items: { key: string; label?: string; color?: string; value: string }[] }) {
  return (
    <div className="border border-[#3a2f4e] divide-y divide-[#3a2f4e] mb-6">
      {items.map((r) => (
        <div key={r.key} className="flex gap-4 px-4 py-3 hover:bg-[#2d2440] transition-colors">
          <span className={`f-type text-[11px] shrink-0 w-32 ${r.color ?? 'text-[#c9c2d6]'}`}>{r.key}</span>
          {r.label && <span className="f-type text-[11px] text-[#e9e4f0] shrink-0 w-20">{r.label}</span>}
          <span className="f-type text-[11px] text-[#9a91ad] leading-5">{r.value}</span>
        </div>
      ))}
    </div>
  );
...
function LangSwitch({ lang, setLang }: { lang: Lang; setLang: (l: Lang) => void }) {
  return (
    <div className="flex border border-[#3a2f4e] mb-8 w-fit">
      {(['ts', 'py'] as Lang[]).map((l, i) => (
        <button
          key={l}
          onClick={() => setLang(l)}
          className={[
            'px-4 py-1.5 f-type text-xs uppercase tracking-[0.1em] transition-colors',
            i === 0 ? 'border-r border-[#3a2f4e]' : '',
            lang === l ? 'bg-[#2d2440] text-[#e9e4f0] font-bold' : 'text-[#9a91ad] hover:text-[#c9c2d6]',
          ].join(' ')}
        >
...
// Every call is now traced in your dashboard`}</Code>
          <Callout type="info">
            Find your API key in the dashboard under <strong className="text-[#e9e4f0]">Settings → API Key</strong> for each project.
          </Callout>
        </>
      ) : (
        <>
...
function SectionSDKTS() {
  ...
  runId:  '...',         // optional — provide your own run ID
      ...
      <P>Returns a drop-in replacement for the Anthropic client. Intercepts every <code className="text-[#b794f4] f-type">messages.create()</code> call, forwards it unchanged, and automatically ingests the trace after the response returns.</P>
...
await anthropic.messages.create({
  ...
  messages: [{ role: 'user', content: 'Summarise...' }],
      ...
      <P>The same drop-in pattern for the OpenAI client. Wraps <code className="text-[#b794f4] f-type">chat.completions.create()</code>, forwards it unchanged, and ingests the trace once the response returns. Pricing for gpt-4o, gpt-4o-mini, o1, o3-mini and others is built in, so cost is captured automatically.</P>
...
await openai.chat.completions.create({
  ...
  _trace: { stepName: 'summarize' },
      ...
      <Callout type="tip">
        Anthropic and OpenAI wrappers share one tracer. Mix providers inside the same <code className="text-[#e9e4f0]">run()</code> and every step still lands under one run_id.
      </Callout>

      <H2>run()</H2>
      <P><strong className="text-[#e9e4f0]">Key concept for multi-step pipelines.</strong> Calling <code className="text-[#b794f4] f-type">anthropic.run()</code> creates a <code className="text-[#b794f4] f-type">TracedRun</code> — a fresh execution context with its own <code className="text-[#b794f4] f-type">run_id</code>. Every step on that run is grouped together in the dashboard.</P>
      <Callout type="warn">
        Without <code className="text-[#e9e4f0]">run()</code>, all calls share the tracer&apos;s single runId and appear as one run. For multi-step workflows, always call <code className="text-[#e9e4f0]">run()</code> at the start of each user request.
      </Callout>
  ...
  console.log('run:', run.runId)       // shared ID for both steps
      ...
      <H2>Streaming</H2>
      <P><code className="text-[#b794f4] f-type">messages.stream()</code> is fully supported. Tokens and latency are captured after the stream ends — zero impact on streaming latency.</P>
...
// trace ingested automatically once stream completes`}</Code>
      ...
      <H2>Naming steps</H2>
      <P>Add <code className="text-[#b794f4] f-type">_trace: {'{ stepName: \'...\' }'}</code> to give a step a human-readable name. Without it, steps are auto-named from the first 4 words of the system prompt.</P>
      <Callout type="tip">
        <strong className="text-[#e9e4f0]">Keep system prompts as static templates.</strong> Dynamic content (user input, runtime values) should live in the messages array, not the system prompt. Cernova uses the system prompt to build a stable semantic fingerprint — dynamic system prompts create duplicate profiles.
      </Callout>
...
await run.messages.create({
  ...
  messages: [{ role: 'user', content: userInput }],
      ...
      <H2>Manual ingest</H2>
      <P>For steps outside the Anthropic client — external APIs, custom models, pre-computed results — use <code className="text-[#b794f4] f-type">tracer.ingest()</code> directly.</P>
...
llm.invoke([...], config={"callbacks": [handler]})`}</Code>
      ...
      <H2>Naming steps</H2>
      <P>Pass <code className="text-[#b794f4] f-type">step_name</code> in config metadata. Without it, the step is labeled from the LangChain model name (e.g. <code className="text-[#b794f4] f-type">ChatAnthropic</code>).</P>
      <Code lang="python">{`chain.invoke(
    {"text": "..."},
    config={"metadata": {"step_name": "summarize"}},
)`}</Code>
      <Callout type="tip">
        <strong className="text-[#e9e4f0]">Keep system prompts as static templates.</strong> Cernova uses the system prompt to build a stable semantic fingerprint. Dynamic system prompts create a new step profile on every call. Put user-specific data in the human message, not the system prompt.
      </Callout>

      <H2>Multi-step pipelines</H2>
      <P>Steps inside a single <code className="text-[#b794f4] f-type">chain.invoke()</code> are automatically grouped into one run. Wrap your workflow in <code className="text-[#b794f4] f-type">RunnableLambda</code> and pass config through to each nested <code className="text-[#b794f4] f-type">llm.invoke()</code>:</P>
...
# → both steps share one run_id in the dashboard`}</Code>
      ...
      <H2>Manual ingest</H2>
      <P>For models outside LangChain — external APIs, custom models, pre-computed results — use <code className="text-[#b794f4] f-type">tracer.ingest()</code> directly. Fire-and-forget: posts in a background thread, never blocks.</P>
...
tracer.ingest(
    ...
    output_code   = "The user wants a refund.",
      ...
      <Callout type="info">
        For step fingerprinting to work correctly, include the system prompt as <code className="text-[#e9e4f0]">{`{"role": "system", "content": "..."}`}</code> inside the messages array — or as a top-level <code className="text-[#e9e4f0]">system</code> field. The system prompt is the stable identity of the step.
      </Callout>
      <Table rows={[
...
function SectionDetection() {
  return (
    <div>
      ...
      <div className="border border-[#3a2f4e] divide-y divide-[#3a2f4e] mb-8">
        {[
          ...
          <div key={l.title} className={`flex gap-4 px-4 py-4 border-l-2 ${l.accent} hover:bg-[#2d2440] transition-colors`}>
            <div>
              <div className="f-news font-bold text-sm text-[#e9e4f0] mb-1">{l.title}</div>
              <p className="f-type text-[11px] text-[#9a91ad] leading-5">{l.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <Rows items={[
        ...
        { key: '< 0.75',    label: 'new',     color: 'text-[#b794f4]', value: 'Genuinely new step. A new profile is created with its own baseline.' },
      ]} />
      <Callout type="info">
      ...
      <Callout type="warn">
        <strong className="text-[#e9e4f0]">Never silently enforced.</strong> A freshly learned contract starts <strong className="text-[#e9e4f0]">proposed</strong> — it is checked and logged on live traffic but does not affect scores until you confirm it. Soft drifts (a brand-new enum value, an out-of-range number) are logged as drift signals, never scored.
      </Callout>
      <Rows items={[
        { key: 'observing', color: 'text-[#9a91ad]',   value: 'Fewer than 20 samples. Still learning — not yet checking.' },
      ...
      <Rows items={[
        ...
        { key: 'confirm / reject', label: 'anomaly', color: 'text-[#b794f4]', value: 'Marks an anomaly real or a false alarm. Captured as a label for tuning.' },
      ]} />
...
curl -X POST https://api.cernova.dev/feedback \\
  ...
  -d '{"subject_type":"contract","subject_id":"<step_profile_id>","verdict":"confirm"}'`}</Code>
      ...
      <P>Open any run and click <strong className="text-[#e9e4f0]">✦ Analyze Run</strong>. Cernova sends the full run context — every step, every anomaly score, every condition code — to claude-sonnet-4-6 and returns a structured report.</P>
      <div className="border border-[#3a2f4e] border-l-2 border-l-[#b794f4] bg-[#281f38] px-5 py-4 mb-5">
        <div className="f-type text-[10px] text-[#b794f4] uppercase tracking-widest mb-3">Example output</div>
        <div className="space-y-3 f-type text-[11px] text-[#9a91ad] leading-5">
          <div><span className="text-[#c9c2d6] font-bold">Root cause</span><p className="mt-1">parse-request returned malformed JSON (unclosed bracket). This propagated into enrich-context causing a stall, then crashed generate-response with a null-reference when it attempted to read the entity list.</p></div>
          <div><span className="text-[#c9c2d6] font-bold">Recommendations</span>
      ...
      <Callout type="info">
        Analysis cost is tracked per project in the <strong className="text-[#e9e4f0]">Usage</strong> tab and counts toward your monthly budget.
      </Callout>
    </div>
  );
}
...
function SectionIntegrations() {
  return (
    <div>
      <P>Cernova is a detection layer, not another pane of glass. Traces flow <strong className="text-[#e9e4f0]">in</strong> from what you already run — SDKs, OpenTelemetry, or a one-time import — and anomalies flow <strong className="text-[#e9e4f0]">out</strong> to the tools your team already watches. Slack and Sentry are configured per-project in <strong className="text-[#e9e4f0]">Settings</strong>, no code changes needed.</P>

      <H2>Slack</H2>
      <P>Paste a Slack <a href="https://api.slack.com/messaging/webhooks" className="text-[#b794f4] hover:text-[#cdb9f7] underline underline-offset-4" target="_blank" rel="noreferrer">Incoming Webhook URL</a> into project settings. Cernova posts alerts when:</P>
      <Rows items={[
      ...
      <Callout type="tip">
        Use the <strong className="text-[#e9e4f0]">Test</strong> button in Settings to confirm delivery before going live.
      </Callout>
      ...
      <Rows items={[
        { key: 'Explore → Traces', color: 'text-[#b794f4]', value: 'All LLM calls as transactions. Click any row to see the span waterfall — op:ai.inference root, op:ai.model.invoke child with gen_ai.usage.* attributes.' },
        { key: 'Issues',           color: 'text-[#b794f4]', value: 'Anomaly events grouped by step name. Each issue shows the full condition breakdown, anomaly score, and a link to the run.' },
      ]} />
      ...
      <Callout type="info">
        Performance spans follow <a href="https://opentelemetry.io/docs/specs/semconv/gen-ai/" className="text-[#b794f4] hover:text-[#cdb9f7] underline underline-offset-4" target="_blank" rel="noreferrer">OpenTelemetry GenAI semantic conventions</a> — gen_ai.usage.input_tokens, gen_ai.system: &quot;anthropic&quot; — compatible with Sentry&apos;s native AI monitoring.
      </Callout>

      <H2>Webhooks</H2>
      <P>Send the structured anomaly event to any endpoint — PagerDuty, Opsgenie, n8n, or your own automation. Set a <strong className="text-[#e9e4f0]">Webhook URL</strong> in project Settings; Cernova POSTs this payload whenever an anomaly fires, at the delivery level you choose (critical only, warning + critical, or off).</P>
      <Code lang="json">{`{
  "schema_version": 1,
  "type": "anomaly",
  "event_id": "9f2c…",
  ...
  "codes": [
    { "code": 5001, "name": "latency_iqr_fence", "penalty": 30 }
      ...
      <P>Every request is signed. The <code className="text-[#b794f4] f-type">X-Cernova-Signature</code> header is <code className="text-[#b794f4] f-type">sha256=</code> followed by the HMAC-SHA256 of the raw request body, keyed with your project&apos;s signing secret (shown in Settings once a URL is saved). Verify it before trusting the payload:</P>
...
def verify(secret: str, body: bytes, header: str) -> bool:
    ...
    return hmac.compare_digest(expected, header)`}</Code>
      <Callout type="info">
        The payload is versioned via <code className="text-[#e9e4f0]">schema_version</code> — fields may be added; renames or removals bump the version. Use the <strong className="text-[#e9e4f0]">Test</strong> button in Settings to fire a synthetic event at your endpoint.
      </Callout>

      <H2>OpenTelemetry</H2>
      <P>Already emitting OpenTelemetry GenAI spans — via <a href="https://github.com/traceloop/openllmetry" className="text-[#b794f4] hover:text-[#cdb9f7] underline underline-offset-4" target="_blank" rel="noreferrer">OpenLLMetry</a>, Traceloop, or native instrumentation? Point your OTLP exporter at Cernova and every LLM span flows into the same detection pipeline — no SDK, no code change. Cernova reads the <a href="https://opentelemetry.io/docs/specs/semconv/gen-ai/" className="text-[#b794f4] hover:text-[#cdb9f7] underline underline-offset-4" target="_blank" rel="noreferrer">GenAI semantic conventions</a> and ignores non-LLM spans.</P>
...
OTEL_EXPORTER_OTLP_HEADERS=Authorization=Bearer <CERNOVA_API_KEY>`}</Code>
      <P>Traces post to <code className="text-[#b794f4] f-type">/v1/traces</code> (OTLP/HTTP, JSON or protobuf). The span&apos;s <code className="text-[#b794f4] f-type">trace_id</code> becomes the Cernova run, each GenAI span becomes a step, and per-step fingerprinting and anomaly detection run exactly as they do for SDK ingests.</P>
      <Callout type="info">
      ...
      <H2>Vercel AI SDK</H2>
      <P>The <a href="https://sdk.vercel.ai" className="text-[#b794f4] hover:text-[#cdb9f7] underline underline-offset-4" target="_blank" rel="noreferrer">Vercel AI SDK</a> emits OpenTelemetry telemetry natively. Enable it per call and route the exporter to Cernova — no wrapper needed.</P>
      <Code>{`const result = await generateText({
  model: openai('gpt-4o-mini'),
  prompt: '...',
  experimental_telemetry: { isEnabled: true },
})`}</Code>
      <P>With <code className="text-[#b794f4] f-type">OTEL_EXPORTER_OTLP_ENDPOINT</code> and the Authorization header set as above, every generation is traced and scored automatically.</P>

      <H2>Langfuse import</H2>
      <P>Already running on Langfuse? Import your generation history to warm per-step baselines in minutes instead of waiting for live traffic. Cernova pulls your <code className="text-[#b794f4] f-type">GENERATION</code> observations, backdates them to their original timestamps, and replays them through the same pipeline — building step profiles and baselines with alerts suppressed, so backfilling months of traffic fires no stale notifications.</P>
  ...
  -d '{
    ...
    "host": "https://cloud.langfuse.com"
      ...
      <P>Credentials are validated immediately — bad keys return <code className="text-[#b794f4] f-type">400</code> — then the import runs in the background. Once a step crosses 20 imported calls, statistical detection activates automatically.</P>
      <Callout type="info">
        Imported calls are tagged <code className="text-[#e9e4f0]">source=langfuse</code> and deduplicated on their Langfuse observation id, so you can re-run the import any time without creating duplicates.
      </Callout>

      <H2>LangSmith import</H2>
      <P>Already on LangSmith? Same warm-start, different source. Cernova queries your <code className="text-[#b794f4] f-type">llm</code> runs, backdates them to their original timestamps, and replays them through the pipeline with alerts suppressed — so months of history seed per-step baselines without firing stale notifications.</P>
  ...
  -d '{
    ...
    "host": "https://api.smith.langchain.com"
      ...
      <P>Add <code className="text-[#b794f4] f-type">"session_ids": ["..."]</code> to scope the import to specific LangSmith projects; omit it to pull every project the key can see. As with Langfuse, credentials are validated immediately — bad keys return <code className="text-[#b794f4] f-type">400</code> — then the import runs in the background.</P>
      <Callout type="info">
  ...
  return (
    <div className="min-h-screen bg-[#201a2b] text-[#e9e4f0]" style={{ fontFamily: 'var(--font-news), Georgia, serif' }}>
      <style>{`
        ...
        ::selection { background: #d9c964; color: #201a2b; }
      `}</style>

      {/* Masthead */}
      <nav className="fixed top-0 left-0 right-0 z-50 border-b-2 border-[#e9e4f0] bg-[#201a2b]">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 f-news font-bold text-xl tracking-tight text-[#e9e4f0]">
            <svg width={17} height={17} viewBox="0 0 24 24" aria-hidden="true" className="shrink-0">
              <polygon points="12,2 20,9 12,22 4,9" fill="#b794f4" fillOpacity="0.22" stroke="#b794f4" strokeWidth="1.6" strokeLinejoin="miter" />
              <path d="M4 9 H20 M12 2 V22" stroke="#b794f4" strokeWidth="0.9" opacity="0.75" fill="none" />
            </svg>
            Cernova
          </Link>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-16 pb-24 flex gap-0">
        ...
        {/* Sidebar */}
        <aside className="hidden lg:block w-52 shrink-0 pt-12 border-r border-[#3a2f4e]">
          <div className="sticky top-24 pr-6">
            <p className="f-type text-[10px] text-[#7c7291] uppercase tracking-widest mb-4">Documentation</p>
              ...
              {SECTIONS.map((s) => (
                <button
                  ...
                  className={[
                    ...
                    active === s.id
                      ? 'border-[#b794f4] bg-[#2d2440]'
                      : 'border-transparent hover:border-[#4a3d63] hover:bg-[#2d2440]',
                  ].join(' ')}
                >
                  <div className={`f-type text-xs ${active === s.id ? 'text-[#e9e4f0]' : 'text-[#b3abc4]'}`}>{s.label}</div>
                  <div className="f-type text-[10px] text-[#7c7291] mt-0.5">{s.sub}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <main className="flex-1 min-w-0 pt-12 pl-0 lg:pl-12 max-w-2xl">
          ...
          {/* Section header */}
          <div className="mb-10 pb-6 border-b border-[#3a2f4e]">
            <p className="f-type text-[10px] text-[#7c7291] uppercase tracking-widest mb-2">
              {SECTIONS[idx]?.label}
            </p>
            <h1 className="f-news font-bold text-4xl text-[#e9e4f0]">
          ...
          {/* Bottom pagination */}
          <div className="mt-16 pt-8 border-t border-[#3a2f4e] flex items-center justify-between">
            {idx > 0 ? (
              <button
                onClick={() => setActive(SECTIONS[idx - 1].id)}
                className="f-type text-xs text-[#9a91ad] hover:text-[#e9e4f0] transition-colors"
              >
            ...
            {idx < SECTIONS.length - 1 ? (
              ...
              <Link href="/login" className="f-type text-xs font-bold px-4 py-2 bg-[#e9e4f0] hover:bg-[#b794f4] text-[#201a2b] transition-colors">
                get started free →
              </Link>
            )}
          </div>
